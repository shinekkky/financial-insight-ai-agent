...
* **审计合规设计**：分析参数 Temperature 默认锁定为 0，确保分析过程的严谨性与结果的可追溯性（如需可调）。

## 界面预览

| 首页与系统配置 | 数据概览 |
|:---:|:---:|
| ![首页](screenshots/homepage.png) | ![数据概览](screenshots/data_overview.png) |

**AI 智能分析**

![AI 分析](screenshots/ai_analysis.png)

> 图中数据均来自脱敏后的公开市场财报，仅作功能演示。
...
├── README.md              # 项目文档：即本文件
├── screenshots/           # 界面截图：首页、数据概览、AI 分析
│   ├── homepage.png
│   ├── data_overview.png
│   └── ai_analysis.png
└── data/
    ├── audit_test_data.xlsx          # 测试数据：脱敏后的企业财务报表 (基于真实市场数据转换)
    └── 贵州茅台(600519)财务报表.xlsx  # 真实公开市场财报，用于功能演示
```
...
## 数据与免责声明
* `data/` 目录中的财务报表均来源于公开市场信息（如贵州茅台 600519 已披露财报）或由其转换生成的脱敏数据。
* 本项目仅作技术能力演示与交流，分析结论由大语言模型生成，可能存在偏差，**不构成任何审计意见、投资建议或决策依据**。
* 请在合规前提下使用，并自行核实数据与分析结果的准确性。
